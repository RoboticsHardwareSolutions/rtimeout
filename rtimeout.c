...
#include "stdio.h"
    ...
    defined(STM32F765xx) || defined(STM32G474xx) || defined(STM32G0B1xx)
...
inline int rt_timed_out(rt *instance) {
  ...
  int result =
      instance->timeout > 0 && elapsed_time > instance->timeout ? 1 : 0;
...
#if defined(RTIMEOUT_FOR_WINDOWS) || defined(RTIMEOUT_FOR_UNIX) ||             \
    defined(RTIMEOUT_FOR_APPLE)
...
inline int rt_timed_out(rt *instance) {
  ...
  uint32_t elapsed_time =
      (uint32_t)((current_time.tv_sec - instance->start_time.tv_sec) * 1000000 +
